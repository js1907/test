...
module.exports = () => {
    ...
    route.get('/search', (req, res) => {
        ...
        const keywordStr = `select  *  from product,shop where product.shop_id=shop.shop_id and concat(product.product_name,product.product_detail,shop.shop_name,shop.shop_address) like '%${keyWord}%'`;
        const hotStr = `select  *  from product,shop where product.shop_id=shop.shop_id and concat(product.product_name,product.product_detail,shop.shop_name,shop.shop_address) like '%${keyWord}%' order by product_comment_num desc`;
        const priceUpStr = `select  *  from product,shop where product.shop_id=shop.shop_id and concat(product.product_name,product.product_detail,shop.shop_name,shop.shop_address) like '%${keyWord}%' order by product_price asc`;
        const priceDownStr = `select  *  from product,shop where product.shop_id=shop.shop_id and concat(product.product_name,product.product_detail,shop.shop_name,shop.shop_address) like '%${keyWord}%' order by product_price desc`;
        ...
        if (keyWord != '') {
            if (hots != '' && hots != undefined && hots=='true') {
                console.log("执行默认列表1");
                getSearchDatas(hotStr, res);
            } else if (priceUp != '' && priceUp != undefined && priceUp=='true') {
                console.log("执行默认列表2");
                getSearchDatas(priceUpStr, res);
            } else if (priceDown != '' && priceDown != undefined && priceDown=='true') {
                console.log("执行默认列表3");
    ...
    */
    ...
    route.post('/reg', (req, res) => {
        ...
        let regMobile = mObj.regMoblie;
        regPasswd = common.md5(regPasswd + common.MD5_SUFFXIE);
        const insUserInfo = `INSERT INTO user(user_name,login_password,user_number) VALUES('${regName}','${regPasswd}','${regMobile}')`;
